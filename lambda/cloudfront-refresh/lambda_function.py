...
def lambda_handler(event, context):
    ...
    # Get the current URI and querystring
    current_uri = request.get("uri", "/")
    current_qs = request.get("querystring", "")
    
    # Parse querystring to get the state parameter
    parsed_qs = urllib.parse.parse_qs(current_qs)
    state = parsed_qs.get("state", [None])[0]
    
    # Determine the final redirect URL
    redirect_url = CONTENT_ROOT
    
    # If we're handling a /refresh request
    if current_uri.startswith("/refresh"):
        # The state parameter should contain the original path
        if state:
            try:
                state_decoded = urllib.parse.unquote(state)
                # Security: Prevent open redirects by blocking absolute URLs
                if state_decoded.startswith("http://") or state_decoded.startswith("https://"):
                    ...
                    if not state_decoded.startswith("/"):
            ...
            # No state parameter, redirect to root
            redirect_url = CONTENT_ROOT
    else:
        # For non-refresh paths, preserve the current URI and querystring
        redirect_url = f"{CONTENT_ROOT}{current_uri}"
        if current_qs:
            redirect_url += f"?{current_qs}"
    ...
    # Prepare Basic auth for Cognito
    ...
    # Build cookies with same attributes as sign-in
    ...
    # Only update refresh token if Cognito provided a new one
    ...
    logger.info(f"Token refresh successful. Redirecting to: {redirect_url}")
...
def call_cognito_refresh(client_id: str, refresh_token: str, basic_auth: str) -> dict:
    """
    Call Cognito's token endpoint to refresh tokens
    """
    payload = {
...
# Load JWKS for token validation (optional)
