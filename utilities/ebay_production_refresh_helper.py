#!/usr/bin/env python3
"""
One-time helper for obtaining an eBay PRODUCTION refresh token and optionally
storing it in AWS Secrets Manager.

Usage examples:

1) Print the production consent URL:
...
Actual values:
python3 ebay_production_refresh_helper.py \
  --client-id XXX-PRD-abcde123-45678fab01 \
...
python3 ebay_production_refresh_helper.py \
  --client-id XXX-PRD-abcde123-45678fab01 \
  --client-secret PRD-abcde123-45678fab01-0123 \
  --runame RuName-Lambda-abcdef \
...
def exchange_code_for_tokens(client_id: str, client_secret: str, runame: str, auth_code: str) -> dict:
    ...
    try:
        ...
        raise SystemExit(f"Unable to reach eBay production token endpoint: {e}") from e
...
def main() -> None:
    ...
    parser.add_argument("--runame", required=True, help="Your eBay Production RuName")
    ...
    if args.print_consent_url or not args.auth_code:
        ...
        print("Open this URL in a browser and sign in with your PRODUCTION eBay user:\n")
        print(consent_url)
    ...
    if args.write_secret:
        ...
        print("\nStore this refresh_token in AWS Secrets Manager under the production secret used by your Lambda.")
